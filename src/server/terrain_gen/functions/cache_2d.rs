...
use crate::server::terrain_gen::func_deserialize::{
    CacheFnHelper, DensityFnOutline, DensityFnOutlineType,
};

use super::super::super::LudiChunkLoader;
use super::{DensityArg, DensityFn, DensityFnArgs};
use ahash::{AHashMap, AHasher};
use serde::Deserialize;

#[derive(Debug)]
pub struct Cache2DFn {
    argument: DensityArg,
    hash: u64,
}

impl<'de> Deserialize<'de> for Cache2DFn {
...
impl DensityFn for Cache2DFn {
    #[inline]
    fn compute(&self, args: &mut DensityFnArgs) -> f64 {
        /*if args.column_cache_passthrough {
            return self.argument.compute(args);
        }
        let packed_coord = LudiChunkLoader::pack_coords((args.block_x, args.block_z));
        if args.column_cache_pos != Some(packed_coord) {
            args.column_cache.borrow_mut().clear();
            args.column_cache_pos = Some(packed_coord);
            let result = self.argument.compute(args);
            args.column_cache.borrow_mut().insert(self.hash, result);
            result
        } else {
            let cache_lookup = args.column_cache.borrow().get(&self.hash).cloned();
            if let Some(result) = cache_lookup {
                // Return cached value if already computed
                result
            } else {
                // Add computed value to cache if not already computed
                let result = self.argument.compute(args);
                args.column_cache.borrow_mut().insert(self.hash, result);
                result
            }
    ...
    // No compute_slice means this density function is the last
    #[inline]
    ...
    fn generate_state(&self, dimension: &str, outline: &mut DensityFnOutline) {
        outline.new_stack_frame(DensityFnOutlineType::Cache2D);
